...
{{- end}}
      ...
      // Create custom fetcher with CSRF token support
...
func NewPlaygroundHandler(title, endpoint string, opts ...GraphiqlConfigOption) http.HandlerFunc {
	data := GraphiqlConfig{
		Title:                title,
		Endpoint:             endpoint,
		...
		// https://www.jsdelivr.net/package/npm/react?tab=files
...
func NewPlaygroundHandlerWithHeaders(
	title, endpoint string,
	fetcherHeaders, uiHeaders map[string]string,
) http.HandlerFunc {
	return NewPlaygroundHandler(
		title,
		endpoint,
